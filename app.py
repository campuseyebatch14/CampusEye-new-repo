...
def index():
    try:
        # Fetch students but exclude embeddings to save RAM
...
def edit_student(student_id):
    """Fixed: Added the required student_id parameter to match templates."""
...
def bulk_upload():
    """Fixed: Added the route referenced in index.html."""
    if request.method == 'GET':
    ...
    try:
        # Lightweight CSV processing instead of Pandas to save RAM
...
def download_report():
    """Fixed: Required by the dashboard to export detection logs."""
    si = io.StringIO()
    cw = csv.writer(si)
